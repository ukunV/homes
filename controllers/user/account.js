...
const postPassword = function (req, res) {
  ...
  mySqlClient.query(Confirm, [req.session.user.userId, userPW], function (err, row) {
    if (row[0]) {
      res.send(
        '<script type="text/javascript">window.location="/account/management";</script>',
        );
    } else {
      res.send(
...
const postChangePw = function (req, res) {
  ...
  if(pw.length > 3){
    ...
    res.send('<script type="text/javascript">alert("비밀번호를 4자리 이상으로 설정하세요."); window.history.back();</script>');
