...
var tenant = function (req, res) {
  if (req.session.user) {
    const tenantInfoSql = `select payment_month_day, payment_cash, payment_type, payment_month_ok, building_name, building_addr, roomNum, u.name as hostName, u.tel as hostTel, bank_account, v.name as managerName, v.tel as managerTel, date_format(begin_date,'%Y-%m-%d') as begin_date, date_format(end_date,'%Y-%m-%d') as end_date from user u join buildings b on u.user_id=b.hostID join room r on b.buildingNum = r.buildNum join (select * from user) v on v.user_id=b.managerID where r.tenantID = ?;`;
